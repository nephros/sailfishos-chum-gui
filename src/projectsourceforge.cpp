...
static QString reqAuth{QStringLiteral("bearer " SOURCEFORGE_TOKEN)};
static QString reqUrl{QStringLiteral("https://sourceforge.net/rest")};
...
static bool parseUrl(const QString &u, QString &project) {
    QUrl url(u);
    QString host = url.host();
    QStringList path = url.path().split('/', QString::SkipEmptyParts);

    if (host != QStringLiteral("sourceforge.net") || path.length() != 2)
        return false;
...
ProjectSourceForge::ProjectSourceForge(const QString &url, ChumPackage *package) : ProjectAbstract(package) {
    bool ok = parseUrl(url, m_project);
    if (!ok) {
        qWarning() << "Shouldn't happen: ProjectSourceForge initialized with incorrect service" << url;
        return;
    }
    ...
    m_package->setUrlIssues(QStringLiteral("https://sourceforge.net/p/%2/issues").arg(m_project));
    // urldiscussion is not set as it is not used by all projects

    // fetch information from SourceForge
    fetchRepoInfo();
}

// static
bool ProjectSourceForge::isProject(const QString &url) {
    QString p;
    return parseUrl(url, p);
}


void ProjectSourceForge::fetchRepoInfo() {
  ...
  repository(owner: \"%1\", name:\"%2\") {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply](){
        if (reply->error() != QNetworkReply::NoError) {
            qWarning() << "Failed to fetch repository data for" << this->m_project;
        ...
        else m_package->setUrl(QStringLiteral("https://sourceforge.net/%1/%2").arg(m_org, m_repo));
        ...
        if (vi>=0)
            m_package->setUrlForum(QStringLiteral("https://sourceforge.net/%1/%2/discussion").arg(m_org, m_repo));
...
void ProjectSourceForge::issue(const QString &issue_id, LoadableObject *value) {
...
void ProjectSourceForge::issues(LoadableObject *value) {
...
void ProjectSourceForge::release(const QString &release_id, LoadableObject *value) {
...
void ProjectSourceForge::releases(LoadableObject *value) {
